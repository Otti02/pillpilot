  ...
  group('MedicationController', () {
    ...
    group('Happy Path Tests', () {
      test('loadMedications should update state with medications', () async {
        ...
        verify(
          mockNotificationService.scheduleMedicationNotification(
      ...
      test(
        'createMedication should create medication and update state',
        () async {
          // ARRANGE
          when(
            mockMedicationService.createMedication(
              'Aspirin',
              '100mg',
      ...
      test(
        'updateMedication should update medication and reload list',
        () async {
          // ARRANGE
          ...
          when(
            mockMedicationService.updateMedication(testMedication),
          ...
          // ACT
          final result = await medicationController.updateMedication(
            testMedication,
          );
          ...
          expect(medicationController.state.medications.length, 1);
          expect(
            medicationController.state.medications.first.name,
            'Updated Aspirin',
          );
          verify(
            mockNotificationService.scheduleMedicationNotification(
      ...
      test(
        'deleteMedication should delete medication and reload list',
        () async {
          // ARRANGE
          when(
            mockMedicationService.deleteMedication('1'),
          ).thenAnswer((_) async => {});
          ...
          verify(
            mockNotificationService.cancelMedicationNotifications('1'),
          ).called(1);
        },
      );

      test(
        'toggleMedicationCompletion should update completion status',
        () async {
          // ARRANGE
          final completedMedication = testMedication.copyWith(
            isCompleted: true,
          );
          ...
          expect(medicationController.state.medications.length, 1);
          expect(
            medicationController.state.medications.first.isCompleted,
            true,
          );
    ...
    group('Error Handling Tests', () {
      test('loadMedications should handle network errors gracefully', () async {
        ...
        expect(medicationController.state.isLoading, false);
        expect(
          medicationController.state.error,
          'Bitte überprüfen Sie Ihre Internetverbindung.',
        );
      });

      test(
        'loadMedications should handle validation errors gracefully',
        () async {
          // ARRANGE
          when(
            mockMedicationService.getMedications(),
      ...
      test(
        'createMedication should handle service errors and rethrow',
        () async {
          // ARRANGE
          when(
            mockMedicationService.createMedication(
      ...
      test(
        'updateMedication should handle service errors and rethrow',
        () async {
          // ARRANGE
          when(
            mockMedicationService.updateMedication(any),
      ...
      test(
        'deleteMedication should handle service errors and rethrow',
        () async {
          // ARRANGE
          when(
            mockMedicationService.deleteMedication('1'),
      ...
      test(
        'toggleMedicationCompletion should handle medication not found',
        () async {
          // ARRANGE
          when(
            mockMedicationService.getMedicationById('999'),
    ...
    group('Midnight Reset Tests', () {
      ...
      test(
        '_resetAllMedications should reset all medications to not completed',
        () async {
          // ARRANGE
          final completedMedication1 = testMedication.copyWith(
            id: '1',
            isCompleted: true,
          );
          final completedMedication2 = testMedication.copyWith(
            id: '2',
            isCompleted: true,
          );
          ...
          when(mockMedicationService.updateMedication(any)).thenAnswer((
            invocation,
          ) async {
